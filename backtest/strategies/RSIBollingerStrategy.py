...
class RSIBollingerStrategy(Strategy):
    ...
    def log(self, txt):
        ...
        if not self.silent_mode:
            ...
            used_margin = sum(
                (price / self.leverage) * abs(t.size) for t in self.trades
            )
            margin_level_pct = (
                (self.equity / used_margin) * 100 if used_margin > 0 else 1000
            )
            free_margin = self.equity - used_margin
            logger.info(
                f"{dt}, {txt}, Price: {price:.2f}, Equity: {self.equity:.2f}. Used Margin: {abs(used_margin):.2f}. Margin Level: {margin_level_pct:.2f}%. Free Margin: {free_margin:.2f}"
            )
    ...
    def next(self):
        ...
        if current_closed_count > self.last_closed_count:
            ...
            if total_profit > 0:
                self.log(
                    f"TAKE PROFIT AUTO | Closed: {len(newly_closed)} pos | Size: {total_size} | Net Profit: ${total_profit:.2f}"
                )
            else:
                self.log(
                    f"STOP LOSS AUTO | Closed: {len(newly_closed)} pos | Size: {total_size} | Net Loss: ${total_profit:.2f}"
                )
        ...
        if n_trades > 0:
            avg_price = sum(abs(t.size) * t.entry_price for t in trades) / sum(
                abs(t.size) for t in trades
            )
            ...
            if trades[0].is_long:
                if price >= avg_price + self.take_profit_ticks:
                    self.log(
                        f"WIN (LONG) | Size: {total_size_abs} | Profit: ${(price - avg_price) * total_size_abs:.2f}"
                    )
                    for t in trades:
                        t.close()
                    return
                ...
                if price <= avg_price - self.take_profit_ticks:
                    self.log(
                        f"WIN (SHORT) | Size: {total_size_abs} | Profit: ${(avg_price - price) * total_size_abs:.2f}"
                    )
                    for t in trades:
                        t.close()
                    return
        ...
        if margin_level_pct <= 100 and margin_level_pct > 50:
            self.log(
                f"MARGIN CALL: Level {margin_level_pct:.2f}% <= 100%. Equity: ${self.equity:.2f}, Used Margin: ${used_margin:.2f}"
            )

        # Stop Out: broker force-closes all positions at the liquidation level.
        if margin_level_pct <= 50:
            self.log(
                f"LIQUIDATION: Level {margin_level_pct:.2f}% <= 50%. Equity: ${self.equity:.2f}, Used Margin: ${used_margin:.2f}"
            )
            for t in trades:
        ...
        if self.equity < floor_value:
            if not self.max_drawdown_reached:
                self.log(
                    f"MAX DD HIT! Equity ${self.equity:.2f} < ${floor_value:.2f}. Freezing."
                )
                self.max_drawdown_reached = True
        ...
        if n_trades > 0:
            if (
                abs(price - trades[-1].entry_price)
                < self.min_dist_between_entries_ticks
            ):
                return
        ...
        current_size = max(
            self.position_size,
            int(round(self.position_size * (self.martingale_multiplier**n_trades))),
        )
        ...
        if cost_to_open > (free_margin - self.security_buffer):
            self.log(
                f"NO MARGIN | Req: ${cost_to_open:.2f} | Free: ${free_margin:.2f} | Buffer: ${self.security_buffer}"
            )
            return
        ...
        if can_buy and price < self.bb_lower[-1] and rsi < self.rsi_oversold:
            if n_trades > 0 and (trades[0].is_short or price >= trades[-1].entry_price):
                return
            ...
            self.log(
                f"BUY #{n_trades + 1} (x{current_size}) @ {price:.2f} | SL: {stop_price} | TP: {target_price}"
            )
            ...
            if n_trades > 0 and (trades[0].is_long or price <= trades[-1].entry_price):
                return
            ...
            self.log(
                f"SELL #{n_trades + 1} (x{current_size}) @ {price:.2f} | SL: {stop_price} | TP: {target_price}"
            )
